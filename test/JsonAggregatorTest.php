...
class JsonAggregatorTest extends TestCase
{
    ...
    {
        $jsonAggregator = new JsonAggregator('/path/to/file.json');
        $this->assertTrue(in_array('json', $jsonAggregator->extensions()));
    }
...
YAML;
        $file = $this->getMockBuilder(File::class)
                     ...
                     ->getMock();
        $file->expects($this->once())->method('getContent')->willReturn($json);

        $aggregator = new JsonAggregator('/path/to/json');
        /** @var File $file */
        ...
        $this->assertEquals('part4', $result['data']['json']);
    }
}
